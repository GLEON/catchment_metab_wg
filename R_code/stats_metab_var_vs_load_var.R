...
metab_plot <- dplyr::filter(all_metab, doy > min_doy, doy < max_doy, GPP_SD/GPP < cv_cutoff) %>%
  ...
  dplyr::summarise(var_gpp = var(GPP, na.rm=T),
            var_r = var(R, na.rm =T),
            var_nep = var(NEP, na.rm=T)) %>%
  ungroup()
...
for(i in seasons){
  if(i == 'annual'){
    ...
    gpp_data = plot_data_annual %>%
      select(rbind('var_gpp',predictors)) %>%
      na.omit()
    gpp_corr_matrix = gpp_data %>% as.matrix() %>% Hmisc::rcorr()

    global_model_gpp = lm(var_gpp ~ ., data = gpp_data)
    ...
    r_data = plot_data_annual %>%
      select(rbind('var_r',predictors)) %>%
      na.omit() %>%
      mutate(var_r = var_r * -1)

    global_model_r = lm(var_r ~ ., data = r_data)
    ...
    nep_data = plot_data_annual %>%
      select(rbind('var_nep',predictors)) %>%
      na.omit()

    global_model_nep = lm(var_nep ~ ., data = nep_data)
...
all_out = bind_rows(gpp_out, r_out, nep_out) %>%
  mutate(metab_response = c(rep('var_gpp',nrow(gpp_out)), rep('var_r', nrow(r_out)), rep('var_nep', nrow(nep_out))))

saveRDS(all_out, 'results/AIC_models/metab_var_aic.rds')

all_out %>%
  kable() %>%
  kable_styling(bootstrap_options = "striped", full_width = F) %>%
  save_kable(file = 'results/AIC_models/metab_var_aic_table.html', self_contained = T)

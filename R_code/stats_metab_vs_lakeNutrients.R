...
load_plot_annual <- dplyr::filter(all_load, doy > min_doy, doy < max_doy) %>%
  ...
  dplyr::summarise(mean_tp_load = mean(TP_load / Volume..m3., na.rm=T),
                   ...
                   mean_lake_tn = mean(TN, na.rm = T),
                   mean_lake_doc = mean(DOC, na.rm = T),
                   mean_lake_doc_tn = mean((DOC/12) / (TN/14), na.rm = T),
                   mean_lake_doc_tp = mean((DOC/12) / (TP/31), na.rm = T),
                   mean_lake_tn_tp = mean((TN/14) / (TP/31), na.rm = T)) %>%
  ungroup()
...
shapiro.test(plot_data_annual$mean_r * -1)
...
# in-lake stoich
shapiro.test(plot_data_annual$mean_lake_doc_tp)
shapiro.test(plot_data_annual$mean_lake_doc_tn)
shapiro.test(plot_data_annual$mean_lake_tn_tp)

shapiro.test(sqrt(plot_data_annual$mean_lake_doc_tp))
shapiro.test(log10(plot_data_annual$mean_lake_tn_tp))
...
table_out = all_out %>%
  select('(Intercept)', mean_lake_doc, mean_lake_tn, mean_lake_tp, mean_lake_doc_tn, mean_lake_doc_tp, mean_lake_tn_tp, AICc, metab_response) %>%
  mutate_if(.predicate = is.numeric, .funs = round_tab) %>%
  imputeTS::na_replace(fill = '')

tab_df(x = table_out,
       col.header = c('Intercept', 'DOC', 'log10(TN)', 'log10(TP)', 'DOC:TN', 'sqrt(DOC:TP)', 'log10(TN:TP)', 'AIC', 'Response Variable'),
       alternate.rows = T, file = 'tables/inlake_metab_AIC_table.doc')

# copying for transformations in table output
# plot_data_annual <- plot_data_annual %>%
#   mutate(mean_gpp = log10(mean_gpp),
#          mean_r = sqrt(mean_r * -1),
#          mean_tp_load = log10(mean_tp_load),

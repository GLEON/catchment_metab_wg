...
plot_data_annual <- plot_data_annual %>%
  mutate(mean_constituent_load = mean_tp_load + mean_tn_load + mean_doc_load) # all consitutents loaded
...
shapiro.test(plot_data_annual$mean_constituent_load)
...
shapiro.test(log10(plot_data_annual$mean_constituent_load))
...
plot_data_annual <- plot_data_annual %>%
  mutate(mean_gpp = log10(mean_gpp),
         ...
         mean_constituent_load = log10(mean_constituent_load))
...
for(i in seasons){
  if(i == 'annual'){
    ...
    predictors = c('mean_constituent_load', 'mean_doc_tp_load', 'mean_doc_tn_load', 'mean_tn_tp_load')
...
saveRDS(all_out, 'results/AIC_models/metab_total_constituent_aic.rds')

all_out %>%
  kable() %>%
  kable_styling(bootstrap_options = "striped", full_width = F) %>%
  save_kable(file = 'results/AIC_models/metab_total_constituent_aic_table.html', self_contained = T)
